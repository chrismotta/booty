<?php
	...
	class iWoopAPI extends Component
	{
		// uses orangear.com plattform
		const URL = 'http://api.iwoop.com/affiliate/offer/findAll/?approved=1';
			...
			foreach ( $response->offers AS $ext_id => $campaign )
			{
				...
				if ( $campaign->Preview_url )
				{
					...
					if ( $campaign->APP_ID )
					{
						...
						{
							$packageIds['ios'] = ApiHelper::cleanAppleId($campaign->APP_ID);
						}
				...
				{
					$packageIds = [
						'ios' => ApiHelper::cleanAppleId($campaign->APP_ID)
					];
				}				
				else
				{
