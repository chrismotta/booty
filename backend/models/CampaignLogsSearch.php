...
class CampaignLogsSearch extends CampaignLogs
{
    ...
    public function searchCsv($daysBefore=4) {
        ...
        $query->select([
            ...
            'IF(cluster_id IS NULL, " ", cluster_id) as cluster_id',
            'IF(F_ClusterLogs.cluster_name IS NULL, " ", F_ClusterLogs.cluster_name) as cluster_name', 
            'IF(Affiliates_id IS NULL, " ", Affiliates_id) as affiliate_id', 
            'IF(Affiliates_name IS NULL, " ", Affiliates_name) as affiliate_name', 
            'IF(F_CampaignLogs.D_Campaign_id IS NULL, " ", F_CampaignLogs.D_Campaign_id) as campaign_id', 
            'IF(D_Campaign.name IS NULL, " ", D_Campaign.name) as campaign_name',  
            'IF(Publishers_id IS NULL, " ", Publishers_id) as publisher_id',
            'IF(Publishers_name IS NULL, " ", Publishers_name) as publisher_name',  
            'IF(F_ClusterLogs.D_Placement_id IS NULL, " ", F_ClusterLogs.D_Placement_id) as placement_id', 
            'IF(D_Placement.name IS NULL, " ", D_Placement.name) as placement_name', 
            'IF(pub_id IS NULL, " ", pub_id)',
            'IF(subpub_id IS NULL, " ", subpub_id)',
            'IF(imp_status IS NULL, " ", imp_status)',
            'IF(ceil(sum(if(clicks>0,imps/clicks,imps))) IS NULL, 0, ceil(sum(if(clicks>0,imps/clicks,imps)))) as imps',
            'IF(count(click_id) IS NULL, 0, count(click_id)) as clicks',
            'IF(count(conv_time) IS NULL, 0, count(conv_time)) as convs',
            'IF(sum(revenue) IS NULL, 0, sum(revenue)) as revenue',
            'IF(sum(if(clicks>0, cost/clicks, cost)) IS NULL, 0, sum(if(clicks>0, cost/clicks, cost))) as cost',
            'IF(sum(revenue) - sum(if(clicks>0, cost/clicks, cost)) IS NULL, 0, sum(revenue) - sum(if(clicks>0, cost/clicks, cost))) as profit',
            ]);
